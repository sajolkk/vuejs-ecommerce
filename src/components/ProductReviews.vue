<template>
    <div id="product-detail" class="product-area col-md-12 px-3 mt-2 mb-5">
        <div class="row" v-if="reviews" >
            ...
            <div class="col-md-12">
                <div class="row">
                    <div class="col-md-12 mt-2" id="questions" v-if="product.reviews">
                        <h5 class="mt-2 mx-1 bg-white mb-0"> 
                            ...
                            <span class="d-block mb-2 py-3 px-3" >
                        ...
                        <div class="mx-1 bg-white px-4 py-2 fw-bold ques-loop rounded mt-1" v-for="review in reviews.data" :key="review.review_id" >
                            <div>
                    ...
                    <div class="col-md-12" >
                        <div class="row row mx-1 mt-3 bg-white rounded py-2">
                            <div class="col-md-12">
                                <div v-if="reviews.last_page>1" class="d-inline-block" >
                                    <div class="d-inline-block" v-if="reviews.current_page == 1">
                                        <button class="btn btn-sm btn-success me-2 mt-1 page-btn" >PREV</button>
                                    </div>
                                    <div class="d-inline-block" v-else>
                                        <button class="btn btn-sm btn-success me-2 mt-1 page-btn" @click="paginationFilterPro(reviews.current_page-1)" >PREV</button>
                                    </div> 
                                    <div class="d-inline-block" v-for="page in reviews.last_page" :key="page">
                                        <button v-if="(page>(reviews.current_page-4)) && (page<(reviews.current_page+4))" class="btn btn-sm link-btn me-2 mt-1" :class="page == reviews.current_page? 'link-active':''"  @click="paginationFilterPro(page)"  >{{ page }}</button>
                                    </div>
                                    <div class="d-inline-block" v-if="reviews.current_page == reviews.last_page">
                                        <button class="btn btn-sm btn-success mt-1 page-btn" >NEXT</button>
                                    </div>
                                    <div class="d-inline-block" v-else>
                                        <button class="btn btn-sm btn-success mt-1 page-btn" @click="paginationFilterPro(reviews.current_page+1)" >NEXT</button>
                                    </div>
                                </div>
                                <div class="page-number d-inline-block float-end">
                                    <p class="mt-1 mb-0" ><small class="fw-bold page-btn">Showing {{ reviews.from }} to {{ reviews.to }} of {{ reviews.total }} ({{ reviews.last_page }} {{ reviews.last_page>1? 'Pages':'Page' }})</small></p>
                                </div>
                            </div>
                        </div>
                    </div>
...
export default{
    ...
    methods:{
        getReview(){
            ...
            this.$store.dispatch('getReviews',formData).then(response => {
        ...
        msgSubmit(){
            ...
            if (this.modalTitle == 'review') {
                ...
                if (this.form.msg.length) {
                    ...
                    this.$store.dispatch('reviewStore',questionData).then(response => {
                        if (response.status == 201) {
                            this.commentModalShow = '';
                            this.$swal({
                                title: 'success!',
